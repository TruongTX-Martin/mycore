...
import com.simicart.core.base.network.androidquery.callback.AjaxCallback;
import com.simicart.core.base.network.androidquery.callback.AjaxStatus;
import com.simicart.core.base.network.androidquery.util.AqueryUtil;
...
public class CoreRequest extends AsyncTask<Void, Void, String> {
	...
	public void request() {
		...
		requestWithAquery(Config.getInstance().getBaseUrl() + url_action);
	}

	private void requestWithAquery(String url) {
		AjaxCallback<JSONObject> callback = new AjaxCallback<JSONObject>() {
			@Override
			public void callback(String url, JSONObject object,
					AjaxStatus status) {
				excuteResult(object.toString(), status);
			}
		};
		callback.header("Content-Type",
				"application/x-www-form-urlencoded; charset=UTF-8");
		callback.header("Cookie", AqueryUtil.getInstance().getCookie());
		callback.header("Token", Config.getInstance().getSecretKey());
		AQuery query = new AQuery(SimiManager.getIntance().getCurrentActivity());
		mHashMap.clear();
		mHashMap.put("data", mJsonParams);
		query.ajax(url, mHashMap, JSONObject.class, callback);
		Log.e("Request:===============>", url + mJsonParams.toString());
	}
	...
	private void handleErrorVolley(VolleyError error, String url) {
		Log.e("CoreRequest-HandleVolleyError:", error + "");
	...
	private void excuteResult(String result, AjaxStatus status) {
		CoreResponse response = new CoreResponse();
		if (response.parse(result)) {
			mDelegate.callBack(response, true);
		} else {
		...
		if (status.getHeaders().size() > 0) {
			if (Config.getInstance().getCookie().equals("")) {
				for (Header header : status.getHeaders()) {
					if (header.getName().contains("Set-Cookie")) {
						Config.getInstance().setCookie(header.getValue());
					}
				}
			}
		}
		
		if (!Config.getInstance().getCookie().equals("") && status.getCookies().size() > 0) {
			for (Cookie cookie : status.getCookies()) {
				AqueryUtil.getInstance().setCookie(cookie.getName() + "=" + cookie.getValue());
			}
		}
	}
