...
public class Product extends SimiEntity {
	...
	public void setCacheBooking(Booking cacheBooking) {
		this.cacheBooking = cacheBooking;
	}
	public Booking getCacheBooking() {
		if(cacheBooking == null) {
			try {
				if(mJSON.has("booking_config")) {
					JSONObject obj_booking = mJSON.getJSONObject("booking_config");
					cacheBooking = new Booking();
					cacheBooking.setJsonObject(obj_booking);
				}
			} catch (Exception e) {
				Log.e("Product-getCacheBooking:", e.getMessage());
			}
		}
		return cacheBooking;
	}
