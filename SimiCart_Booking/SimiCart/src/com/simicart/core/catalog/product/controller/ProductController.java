...
public class ProductController extends SimiController implements
	...
	public void addtoCart() {
		...
		CacheBooking Cachebooking = InstanceBooking.getInstance().getCacheBookingFromProductId(mID);
		if(getProductFromCollection().getBooking() != null) {
			if(Cachebooking == null){
				onBooking(true);
				return;
			}
			if(Cachebooking != null && Cachebooking.getPosition_start() == 0){
				onBooking(true);
				return ;
			}
		}
		...
		CacheBooking booking = InstanceBooking.getInstance()
				.getCacheBookingFromProductId(
						getProductFromCollection().getId());
		if (booking != null) {
			String dateFrom = booking.getDateFrom();
			String dateTo = booking.getDateTo();
			String time_from = booking.getParamTimeFrom();
			String time_to = booking.getParamTimeTo();
			if (Utils.validateString(dateFrom)) {
				model.addParam(InstanceBooking.getInstance().getDATE_FROM(),
						dateFrom);
			}
			if (Utils.validateString(dateTo)) {
				model.addParam(InstanceBooking.getInstance().getDATE_TO(),
						dateTo);
			}
			if (Utils.validateString(time_from)) {
				model.addParam(InstanceBooking.getInstance().getTIME_FROM(),
						time_from);
			}
			if (Utils.validateString(time_to)) {
				model.addParam(InstanceBooking.getInstance().getTIME_TO(),
						time_to);
			}
		}
		model.request();
	...
	// if return true, it mean user choose booking done
	private boolean checkAllBooking() {
		Product product = getProductFromCollection();
