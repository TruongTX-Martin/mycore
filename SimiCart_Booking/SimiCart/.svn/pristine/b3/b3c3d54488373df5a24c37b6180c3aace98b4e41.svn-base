		...
		mModel.setDelegate(new ModelDelegate() {
			...
			public void callBack(String message, boolean isSuccess) {
				...
				if (isSuccess) {
					// mDelegate.updateView(mModel.getCollection());
					if (_changePass) {
						DataLocal.saveData(profile.getName(),
								profile.getEmail(), profile.getNewPass());
